...
public class OAuthMessage {
	...
	/**
	 * Instantiates a new o auth message.
	 *
	 ...
	 * @param URL the uRL
	 * @param parameters the parameters
	 * @param bodyAsStream the body as stream
	 */
	public OAuthMessage(String method, String URL,
			Collection<? extends Map.Entry> parameters, InputStream bodyAsStream) {
		this.method = method;
		this.URL = URL;
        /*somehow the https scheme is being lost on our heroku servers, example, try to access https://app.suitedto.com and
        the URL value that is available here becomes http://app.suitedto.com.  This appears to just be a problem on
        the production servers as the same thing is not happening on local environments.  Use the existence of the
        FORCE_SSL environment variable to set the URL properly here */
        if(Boolean.valueOf(Play.configuration.getProperty("force.ssl"))){
	 ...
	 * This method isn't repeatable, since it consumes and closes
	 * getBodyAsStream.
	 * 
	 ...
	 * caller must close the returned stream, to release the underlying
	...
	public void addRequiredParameters(OAuthAccessor accessor)
		...
		if (signatureMethod == null) {
			signatureMethod = consumer.signatureMethod;
			if (signatureMethod == null) {
		...
		if (pMap.get(OAuth.OAUTH_TIMESTAMP) == null) {
			addParameter(OAuth.OAUTH_TIMESTAMP,
	...
	public String getAuthorizationHeader(String realm) throws IOException {
		...
		if (realm != null) {
			into.append(" realm=\"").append(OAuth.percentEncode(realm))
					.append('"');
		}
		beforeGetParameter();
		if (parameters != null) {
			for (Map.Entry parameter : parameters) {
				...
				if (name.startsWith("oauth_")) {
					...
					into.append(
							OAuth.percentEncode(toString(parameter.getValue())))
							.append('"');
				}
			}
		}
